...
class StatsTab:
    ...
    def create_ui(self):
        ...
        # Main frame
        self.stats_frame = ctk.CTkFrame(self.root, fg_color="transparent", corner_radius=16)
    ...
    def refresh(self):
        ...
        # Stats container
        stats_container = ctk.CTkFrame(self.stats_frame, fg_color=theme.BG_CONTAINER)
        stats_container.pack(fill="both", expand=True, padx=5, pady=(0,10))

        label_font_bold = ctk.CTkFont(size=12, weight="bold")
        r = 0

        # Player level and experience
        ctk.CTkLabel(stats_container, text="Level:", font=label_font_bold).grid(row=r, column=0, sticky="w", padx=10,
                                                                                pady=8)
        ...
        ctk.CTkLabel(stats_container, text=f"{int(self.player.exp)}/{self.player.exp_to_next}",
                     font=ctk.CTkFont(size=14)).grid(row=r, column=1, sticky="w", padx=10, pady=8)
        r += 1

        ctk.CTkLabel(stats_container, text="Total XP:", font=label_font_bold).grid(row=r, column=0, sticky="w", padx=10,
                                                                                   pady=8)
